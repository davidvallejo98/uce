...
if (isset($_GET['buscar'])) {
  ...
  $query = "SELECT reservas.*, 
          ...
          WHERE usuarios.ci LIKE '%$busqueda%' 
             OR recursos.equipo LIKE '%$busqueda%' 
             OR curso LIKE '%$busqueda%' 
             OR usuarios2.nombres LIKE '%$busqueda%' 
             OR usuarios2.apellidos LIKE '%$busqueda%' 
          LIMIT $inicio, $limite";
} else {
  $query = "SELECT reservas.*, 
                 recursos.equipo AS nombre_equipo, 
...
// Calcular total de reservas para paginación
$countQuery = "SELECT COUNT(*) as total FROM reservas";
if (!empty($busqueda)) {
  $countQuery .= " 
      INNER JOIN usuarios ON reservas.ci = usuarios.ci 
      INNER JOIN usuarios AS usuarios2 ON reservas.docente_asignado = usuarios2.ci 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['accion']) || isset($_POST['accion-editar']))) {
  ...
  if (isset($_POST['accion']) && $_POST['accion'] == 'crear') {
    ...
    if ($stmt->execute()) {
      ...
      header("Location: reservas.php?mensaje=Reserva creada");
      exit;
    } else {
  ...
  if (isset($_POST['accion-editar']) && $_POST['accion-editar'] == 'editar') {
    ...
    if ($stmt->execute()) {
      ...
      // Si cambió el recurso, liberamos el anterior y ocupamos el nuevo
      if (trim($codigo_anterior) !== trim($codigo)) {
        ...
        // Marcar nuevo recurso como ocupado
        ...
        $disponibilidad = (isset($_POST['disponibilidad'])) ? intval($_POST['disponibilidad']) : 0;
        $stmtUpdateDisp = $conn->prepare("UPDATE recursos SET disponibilidad = ? WHERE codigo = ?");
        $stmtUpdateDisp->bind_param("is", $disponibilidad, $codigo);
        $stmtUpdateDisp->execute();
        $stmtUpdateDisp->close();
      }

      header("Location: reservas.php?mensaje=Reserva actualizada");
      exit;
    } else {
...
if (isset($_GET['id_reserva'])) {
  ...
  if ($stmt_delete->execute()) {
    ...
    header("Location: reservas.php?mensaje=Reserva eliminada con éxito");
    exit;
  } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_reserva'])) {
  ...
  if ($result->num_rows > 0) {
    ...
    if ($stmt2->execute()) {
      header("Location: reservas.php?mensaje=equipo_entregado");
      exit();
    } else {
...
<div class="container my-5">
  ...
  <form method="GET" class="mb-4">
    <div class="input-group">
      <input type="text" class="form-control" name="buscar" value="<?php echo $busqueda; ?>" placeholder="Buscar por cédula, equipo o curso">
...
">
              ...
              <td>
                ...
                <!-- Botón para activar los botones Editar/Eliminar -->
                <button
                  type="button"
                  class="btn btn-primary btn-sm mt-1"
                  onclick="activarBotones(this)">
                  Activar Acciones
                </button>
                ...
                <form action="reservas.php" method="POST" style="display: inline;">
                  ...
                  <button
                    ...
                    <?= $row['entregado'] == 1 ? 'disabled' : '' ?>>
                    <?= $row['entregado'] == 1 ? 'Entregado ✔️' : 'Entregado' ?>
                  </button>
                </form>
              </td>
                      ...
                      <form action="reservas.php" method="POST">
                        ...
                        <div class="mb-3">
                          ...
                          <select class="form-select" id="disponibilidad<?= $row['id_reserva'] ?>" name="disponibilidad" required>
                            ...
                            <option value="1" <?= $row['disponibilidad_recurso'] == '1' ? 'selected' : '' ?>>Disponible</option>
        ...
        <ul class="pagination">
          <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
            <li class="page-item <?= ($i == $pagina) ? 'active' : '' ?>">
              <a class="page-link" href="?pagina=<?= $i ?><?= ($busqueda ? "&buscar=$busqueda" : "") ?>"><?= $i ?></a>
            </li>
          <?php endfor; ?>
        </ul>
      </nav>
...
<div class="modal fade" id="crearModal" tabindex="-1" aria-labelledby="crearModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="formCrearReserva" action="reservas.php" method="POST">
...
<script>
  ...
  function activarBotones(button) {
    ...
    // Deshabilita el botón de activar si ya se usó (opcional)
